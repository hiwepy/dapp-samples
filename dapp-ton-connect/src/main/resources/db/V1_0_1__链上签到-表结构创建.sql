...
CREATE TABLE `t_config_chain_daily_sign` (
   ...
   `reward_type` int(2) DEFAULT 0 COMMENT '奖励类型（1：现金、2：货币、3...）',
   `amount` decimal(16,4) NOT NULL DEFAULT '0.0000' COMMENT '奖励',
   `created_at` bigint NOT NULL DEFAULT '0' COMMENT '创建时间',
   `updated_at` bigint NOT NULL DEFAULT '0' COMMENT '更新时间',
   PRIMARY KEY (`id`) USING BTREE,
   KEY `idx_day` (`days`) USING BTREE COMMENT '链上签到奖励配置索引'
...
CREATE TABLE `t_chain_daily_sign_log` (
    ...
    `chain_token_id` varchar(50) NOT NULL COMMENT '链token',
    `sign_time` datetime NOT NULL COMMENT '签到时间',
    `reward_type` int(2) DEFAULT 0 COMMENT '奖励类型（1：Ton Coin、2：货币、3...）',
    `reward_amount` decimal(20,8) NOT NULL DEFAULT '0.00000000' COMMENT '签到奖励数量',
    `reward_content` text NULL COMMENT '签到奖励内容',
    `created_at` bigint NOT NULL DEFAULT '0' COMMENT '创建时间',
    `updated_at` bigint NOT NULL DEFAULT '0' COMMENT '更新时间',
    PRIMARY KEY (`id`) USING BTREE,
    KEY `idx_user` (`user_id`,`sign_type`) USING BTREE COMMENT '链上签到记录索引'
